...
def is_game_valid(max_counts_per_colour):
    for key in game_configuration.keys():
        if max_counts_per_colour.get(key, 0) > game_configuration.get(key):
            return False
    return True
...
def analyze_game(line):
    ...
    if is_game_valid(max_counts_per_colour):
        return game_id
    else: 
        return None
